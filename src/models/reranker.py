...
class Reranker:
    def __init__(self, repository, config):
        ...
        self.output_type  = self.config.get("output_type", "probability")
        self.apply_sigmoid = self.config.get("apply_sigmoid", False)

        if self.output_type not in ("probability", "logits"):
            raise ValueError(f"Unsupported reranker output_type: {self.output_type}")
            ...
            try:
                ...
                for score, (idx, cache_key, variant_id) in zip(scores, indices_to_score):
                    ...
                    if self.apply_sigmoid:
                        normalized_score = 1 / (1 + torch.exp(torch.tensor(-raw_score)).item())
                    else:
